...
- User interface available in German, English, Russian, and Ukrainian;
  follows the Total Commander language automatically (see *Language*)

## Installation

Copy `RegHive.wlx64` (64-bit Total Commander) and/or `RegHive.wlx`
(32-bit) plus `reghive.lng` into your TC plugin directory, then add
the plugin under *Configuration → Options → Plugins → Lister plugins* (or
use the included `pluginst.inf`).

## Language

The interface language follows Total Commander automatically.
`reghive.lng` (same folder as the DLL) contains one section per
language; RegHive uses the section whose name matches the last part of
TC's current language file name — `WCMD_DEU.LNG` selects `[deu]`,
`WCMD_RUS.LNG` selects `[rus]`. Included: `[eng]`, `[deu]`, `[rus]`,
`[ukr]`. If the section, a key, or the file is missing, English is used.

To force a language regardless of TC, set it in `reghive.ini` (same
folder as the DLL):
...
Valid values: `auto` (default) or a language code such as `eng`, `deu`,
`rus`, `ukr` (the short forms `en`, `de`, `ru`, `uk` also work).
